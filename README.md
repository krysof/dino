...
当前发行版包含扩展简体中文汉化：461 条精确规则覆盖投币/开始、人物姓名和能力、
全部 41 个 HUD 敌人/Boss 名称、八章标题、关卡地点、开场叙事、过场对白、Boss 台词与
结局文本。509 个中文字形已经转换成 1 bpp 二进制数组并封装在 `dino.data` 中。程序表内
318 条大字体物理行与 41 个 HUD 名称均通过自动完整性审计；实际换行、上下行标点及
混合字体也有回归测试。原版 8×8 英文字块命中后统一以完整 16×16 中文字形替换，不再
缩成难辨认的小字。人物选择卡片改为三行紧凑排版；战斗 HUD 另外识别无后缀玩家
短名，下移血条并把玩家头像纵向补足 8 像素，避免姓名、头像和血条重叠。游戏 Logo、
